...
for (( i = 0; i < ${#list_of_datasets[@]}; i++ )); do
    ...
    # prepare output folder DataSet_dir
    ...
    # print message
    echo_output "Now sorting out unique sources in \"$DataSet_dir\" and copying split_*_spw*_width${width_str}.uvfits"
    
    # read source names
    list_of_unique_source_names=($(ls ../../Level_3_Split/$DataSet_dir/split_*_spw*_width${width_str}.uvfits | perl -p -e 's%.*split_(.*?)_spw[0-9]+_width[0-9kms]+.uvfits$%\1%g' | sort -V | uniq ) )
    ...
    for (( j = 0; j < ${#list_of_unique_source_names[@]}; j++ )); do
        ...
        echo_output cp ../../Level_3_Split/$DataSet_dir/split_"${source_name}"_spw*_width${width_str}.uvfits "${source_name}/"
        cp ../../Level_3_Split/$DataSet_dir/split_"${source_name}"_spw*_width${width_str}.uvfits "${source_name}/"
    done
