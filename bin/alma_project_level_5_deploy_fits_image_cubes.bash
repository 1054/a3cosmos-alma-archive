...
if [[ $# -lt 2 ]]; then
    ...
    echo "Example: "
    echo "    alma_project_level_5_deploy_fits_image_cubes.bash 2013.1.00034.S ../../alma_archive"
    echo "Notes: "
    echo "    This code will copy \"cube.I.image.fits\" files under Level_4_Data_Images "
    echo "    to the path \"<Deploy_Directory>/cubes/\"."
    echo "    A subfolder \"cubes\" will be created under the Deploy_Directory."
    exit
fi
...
for (( i = 0; i < ${#list_image_files[@]}; i++ )); do
    ...
    if ([[ ! -f "${Deploy_dir}/${Subset_dir}/${project_code}/${image_file}" ]] && \
        ...
        if [[ -f "${tclean_params_path}" ]]; then
            echo_output "cp -L \"${tclean_params_path}\" \"${Deploy_dir}/${Subset_dir}/${project_code}/${tclean_params_file}\""
            cp -L "${tclean_params_path}" "${Deploy_dir}/${Subset_dir}/${project_code}/${tclean_params_file}"
        fi
    fi
    ...
    # 
    ref_pix=$(get_fits_header_key "${image_file}" CRPIX3)
    chan_num=$(get_fits_header_key "${image_file}" NAXIS3)
    chan_width=$(get_fits_header_key "${image_file}" CDELT3)
