...
# make Deploy_dir and the "fits_cubes" subdirectory
if [[ "$Deploy_dir" == *"/" ]]; then
    Deploy_dir=$(echo "$Deploy_dir" | sed -e 's%/$%%g')
fi
if [[ ! -d "$Deploy_dir/fits_cubes" ]]; then
    echo_output "mkdir -p \"$Deploy_dir/fits_cubes\""
    mkdir -p "$Deploy_dir/fits_cubes"
fi
if [[ ! -d "$Deploy_dir/fits_cubes" ]]; then
    echo_error "Error! Could not create output directory \"$Deploy_dir/fits_cubes\"! Please check your permission!"
    exit 255
fi
...
echo "Searching for \"Level_4_Data_Images/*/*/split_*_spw*_width*_cube_clean.image.fits\" ..."
list_image_files=($(find Level_4_Data_Images -mindepth 3 -maxdepth 3 -type f \( -name "split_*_spw*_width*_cube_clean.image.fits" -o -wholename "*/DataSet_Merged/*_cube_clean.image.fits" \) ))
...
# check alma_project_meta_table_for_fits_image_cubes.txt, 
# if it exists, then we will append to it (but clear previous project), otherwise initialize one
if [[ -f "${Deploy_dir}/alma_project_meta_table_for_fits_image_cubes.txt" ]]; then
    echo_output "Found existing \"${Deploy_dir}/alma_project_meta_table_for_fits_image_cubes.txt\""
    if [[ -f "${Deploy_dir}/alma_project_meta_table_for_fits_image_cubes.txt.backup" ]]; then
        echo_output "mv \"${Deploy_dir}/alma_project_meta_table_for_fits_image_cubes.txt.backup\" \"${Deploy_dir}/alma_project_meta_table_for_fits_image_cubes.txt.backup.backup\""
        mv "${Deploy_dir}/alma_project_meta_table_for_fits_image_cubes.txt.backup" "${Deploy_dir}/alma_project_meta_table_for_fits_image_cubes.txt.backup.backup"
    fi
    echo_output "mv \"${Deploy_dir}/alma_project_meta_table_for_fits_image_cubes.txt\" \"${Deploy_dir}/alma_project_meta_table_for_fits_image_cubes.txt.backup\""
    mv "${Deploy_dir}/alma_project_meta_table_for_fits_image_cubes.txt" "${Deploy_dir}/alma_project_meta_table_for_fits_image_cubes.txt.backup"
    echo_output "cat \"${Deploy_dir}/alma_project_meta_table_for_fits_image_cubes.txt.backup\" | head -n 1 > \"${Deploy_dir}/alma_project_meta_table_for_fits_image_cubes.txt\""
    cat "${Deploy_dir}/alma_project_meta_table_for_fits_image_cubes.txt.backup" | head -n 1 > "${Deploy_dir}/alma_project_meta_table_for_fits_image_cubes.txt"
    echo_output "cat \"${Deploy_dir}/alma_project_meta_table_for_fits_image_cubes.txt.backup\" | grep -v \'^#\' | grep -v \" ${Project_code} \" >> \"${Deploy_dir}/alma_project_meta_table_for_fits_image_cubes.txt\""
    cat "${Deploy_dir}/alma_project_meta_table_for_fits_image_cubes.txt.backup" | grep -v '^#' | grep -v " ${Project_code} " >> "${Deploy_dir}/alma_project_meta_table_for_fits_image_cubes.txt"
else
    echo_output "Initializing \"${Deploy_dir}/alma_project_meta_table_for_fits_image_cubes.txt\""
    printf "# %-15s %-20s %-25s %8s %12s %12s %15s %15s %15s %15s %11s %11s %11s %15s %15s   %-s\n" \
        ...
        > "${Deploy_dir}/alma_project_meta_table_for_fits_image_cubes.txt"
fi
...
for (( i = 0; i < ${#list_image_files[@]}; i++ )); do
    ...
    if [[ "$dataset_id" == "DataSet_Merged"* ]]; then
        ...
        image_name=$(basename "${image_path}" | perl -p -e 's/[ \n\r]*$//g' | perl -p -e 's/^(split_|output_|combined_|merged_|)(.*_spw[0-9]+)_width.*_cube_clean.image.fits/\2.cube.I.image.fits/g')
    else
        ...
        image_name=$(basename "${image_path}" | perl -p -e 's/[ \n\r]*$//g' | perl -p -e 's/^(split_|output_|)(.*)_width.*_cube_clean.image.fits/\2.cube.I.image.fits/g')
    fi
    
    image_file="${project_code}.member.${mem_ous_id_str}.${image_name}"
    
    # copy fits file
    if [[ ! -f "${Deploy_dir}/fits_cubes/${image_file}" ]] || [[ $overwrite -gt 0 ]]; then
        echo_output "cp \"${image_path}\" \"${Deploy_dir}/fits_cubes/${image_file}\""
        cp "${image_path}" "${Deploy_dir}/fits_cubes/${image_file}"
        ...
        if [[ -f "${pb_image_path}" ]]; then
            echo_output "cp \"${pb_image_path}\" \"${Deploy_dir}/fits_cubes/${pb_image_file}\""
            cp "${pb_image_path}" "${Deploy_dir}/fits_cubes/${pb_image_file}"
        fi
        ...
        if [[ -f "${pbcor_image_path}" ]]; then
            echo_output "cp \"${pbcor_image_path}\" \"${Deploy_dir}/fits_cubes/${pbcor_image_file}\""
            cp "${pbcor_image_path}" "${Deploy_dir}/fits_cubes/${pbcor_image_file}"
        fi
        ...
        if [[ -f "${tclean_params_path}" ]]; then
            echo_output "cp \"${tclean_params_path}\" \"${Deploy_dir}/fits_cubes/${tclean_params_file}\""
            cp "${tclean_params_path}" "${Deploy_dir}/fits_cubes/${tclean_params_file}"
        fi
    fi
    
    # cd into the directory
    Current_dir=$(pwd -P)
    echo_output "cd \"${Deploy_dir}/fits_cubes\""
    cd "${Deploy_dir}/fits_cubes"
    ...
    # 
    ref_pix=$(gethead "${image_file}" CRPIX3)
    chan_num=$(gethead "${image_file}" NAXIS3)
    chan_width=$(gethead "${image_file}" CDELT3)
    ...
    # write to alma_project_meta_table_for_fits_image_cubes.txt
    printf "  %-15s %-20s %-25s %8s %12g %12d %+15.9e %15.9e %15.9e %15g %11g %11g %11g %15.10f %+15.10f   %-s\n" \
        ...
        >> "${Deploy_dir}/alma_project_meta_table_for_fits_image_cubes.txt"
    echo_output "Written to \"${Deploy_dir}/alma_project_meta_table_for_fits_image_cubes.txt\""
done


date +"%Y-%m-%d %Hh%Mm%Ss %Z" > "done_deploying_fits_image_cubes"
echo "pwd: $(pwd)" >> "done_deploying_fits_image_cubes"
echo "args: $@" >> "done_deploying_fits_image_cubes"
