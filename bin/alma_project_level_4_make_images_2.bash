...
width="25km/s"
maximsize=2000
maxlinewidth=-1 # a negative value means using the full bandwidth
overwrite=0
keepfiles=0
select_dataset=()
while [[ $iarg -le $# ]]; do
    ...
    if ([[ "$istr" == "-maxlinewidth" ]] || [[ "$istr" == "-line-velocity-width" ]]) && [[ $((iarg+1)) -le $# ]]; then
        iarg=$((iarg+1)); maxlinewidth="${!iarg}"; echo "Setting maxlinewidth=\"${!iarg}\""
    fi
    ...
    for (( j = 0; j < ${#list_of_unique_source_names[@]}; j++ )); do
        ...
        for (( k = 0; k < ${#list_of_ms_data[@]}; k++ )); do
            ...
            if [[ ! -f "${run_script}" ]]; then
                ...
                echo "    line_velocity_width=${maxlinewidth}, \\"                    >> "${py_script}"
