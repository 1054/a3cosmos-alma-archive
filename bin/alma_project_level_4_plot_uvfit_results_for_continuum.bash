...
if [[ $# -eq 0 ]]; then
    echo "Usage: "
    echo "    alma_project_level_4_plot_uvfit_results_for_continuum.bash Project_code"
    echo "Example: "
    echo "    alma_project_level_4_plot_uvfit_results_for_continuum.bash 2013.1.00034.S"
    echo "Notes: "
    echo "    This code will run uv_fit for all uvt files under Level_4_Data_uvt and output catalog into Level_4_Plot_uvfit_results_for_continuum"
    exit
fi
...
# prepare Level_4_Plot_uvfit_results_for_continuum folder
if [[ ! -d Level_4_Plot_uvfit_results_for_continuum ]]; then 
    echo_output "mkdir Level_4_Plot_uvfit_results_for_continuum"
    mkdir Level_4_Plot_uvfit_results_for_continuum
fi
echo_output "cd Level_4_Plot_uvfit_results_for_continuum"
cd Level_4_Plot_uvfit_results_for_continuum
...
for (( i = 0; i < ${#list_of_datasets[@]}; i++ )); do
    ...
    # check Level_4_Run_uvfit_for_continuum subdirectories
    list_of_source_dirs=($(ls -1d ../Level_4_Run_uvfit_for_continuum/$DataSet_dir/* ) )
    if [[ ${#list_of_source_dirs[@]} -eq 0 ]]; then
        echo_error "Error! \"../Level_4_Run_uvfit_for_continuum/$DataSet_dir/\" does not contain any subdirectories?! Please run Level_4_Run_uvfit_for_continuum first! We will skip this dataset for now."
        continue
    fi
    
    # read source names
    list_of_unique_source_names=($(find ../Level_4_Run_uvfit_for_continuum/$DataSet_dir -mindepth 1 -maxdepth 1 -type d | perl -p -e 's%.*/([^/]+)%\1%g' | sort -V | uniq ) )
    if [[ ${#list_of_unique_source_names[@]} -eq 0 ]]; then
        echo_error "Error! Failed to find \"../Level_4_Run_uvfit_for_continuum/$DataSet_dir/*\" and get unique source names!"
        exit 255
    fi
    ...
    for (( j = 0; j < ${#list_of_unique_source_names[@]}; j++ )); do
        ...
        list_of_uvt_result_files=($(ls -1 ../../../Level_4_Run_uvfit_for_continuum/"$DataSet_dir"/"${source_name}"/run_uvfit_"${source_name}"_spw*_width*_SP/output_point_model_fixed_pos.result.obj_1.txt))
...
# Level_4_Data_uvt
# Level_4_Run_clean
# Level_4_Plot_uvfit_results_for_continuum
# Level_5_Sci
