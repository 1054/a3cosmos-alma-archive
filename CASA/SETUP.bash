...
if [[ -f $(dirname "${BASH_SOURCE[0]}")"/Portable/bin/bin_setup.bash" ]]; then
    CASA_SETUP_BINSETS=$(dirname "${BASH_SOURCE[0]}")"/Portable/bin/bin_setup.bash"
    CASA_SETUP_DIRROOT=$(dirname "${BASH_SOURCE[0]}")"/Portable"
elif [[ -f $(dirname "${BASH_SOURCE[0]}")"/bin/bin_setup.bash" ]]; then
    CASA_SETUP_BINSETS=$(dirname "${BASH_SOURCE[0]}")"/bin/bin_setup.bash"
    CASA_SETUP_DIRROOT=$(dirname "${BASH_SOURCE[0]}")
else
    echo "Error! File not found: $(dirname ${BASH_SOURCE[0]})/Portable/bin/bin_setup.bash"
    return 255
fi
CASA_SETUP_DIRPREF="casa*-"
CASA_SETUP_VERSION="5.6.1"
CASA_SETUP_ARCHIVE="*"

if [[ $# -ge 1 ]]; then
    if [[ "$1" == "clear" ]]; then
        source "${CASA_SETUP_BINSETS}" -var PATH -clear '*casa-*' '*casapy-*' '*/CASA/Portable/*' #-debug
        source "${CASA_SETUP_BINSETS}" -var LD_LIBRARY_PATH -clear '*casa-*' '*casapy-*' '*/CASA/Portable/*' #-debug
        source "${CASA_SETUP_BINSETS}" -var CASALD_LIBRARY_PATH -clear '*casa-*' '*casapy-*' '*/CASA/Portable/*' #-debug
        unset PYTHONPATH
        return 0
    elif [[ "$1" == *"README"* ]]; then
...
if [[ ${#CASA_SETUP_DIRLIST[@]} -eq 0 ]]; then
    ...
    source "${CASA_SETUP_BINSETS}" -var PATH -path "${CASA_SETUP_DIRPATH}/bin" -noop -clear '*casa-*' '*casapy-*' '*/CASA/Portable/*' #-debug
    return 255
else
...
source "${CASA_SETUP_BINSETS}" -var PATH -path "${CASA_SETUP_DIRPATH}/bin" -append -clear '*casa-*' '*casapy-*' '*/CASA/Portable/*' #-debug

source "${CASA_SETUP_BINSETS}" -var LD_LIBRARY_PATH -path "${CASA_SETUP_DIRPATH}/lib" -prepend -clear '*casa-*' '*casapy-*' '*/CASA/Portable/*' #-debug

source "${CASA_SETUP_BINSETS}" -var CASALD_LIBRARY_PATH -path "${CASA_SETUP_DIRPATH}/lib" -prepend -clear '*casa-*' '*casapy-*' '*/CASA/Portable/*' #-debug

if [[ -d "${CASA_SETUP_DIRPATH}/lib/python2.7/site-packages" ]]; then
source "${CASA_SETUP_BINSETS}" -var PYTHONPATH -path "${CASA_SETUP_DIRPATH}/lib/python2.7/site-packages" -prepend -clear '*casa-*' '*casapy-*' '*/CASA/Portable/*' #-debug
